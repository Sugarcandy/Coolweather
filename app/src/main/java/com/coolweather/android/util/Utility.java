    ...
    public static boolean handleCountyResponce(String responce,int cityId )
    {
        if(!TextUtils.isEmpty(responce))
        {
            try
            {
                ...
                for(int i=0;i<allProvinces.length();i++)
                {
                    JSONObject CountyObjecct=allProvinces.getJSONObject(i);
                    County county=new County();
                    county.setCountryName(CountyObjecct.getString("name"));
                    county.setWeatherId(CountyObjecct.getString("weather_id"));
